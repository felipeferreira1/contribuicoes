...
base_a = base
base_a=apply(base_a[,2:56],2,function(x){base_a=x/as.numeric(dias_uteis[,2]);return(base_a)})
...
base=cbind(base[1],apply(base[,2:56],2,function(x) x*(tail(deflator,1)/deflator)))
...
write.csv2(contribuicao,"Contribuicoes concessoes para o total.csv", row.names = F)
