...
#2)Cartao de credito a vista que será retirado do total
serie=c(20655, 20681)
...
base[54] = base[54] - base2[2] - base2[3]
...
base_a = base
base_a=apply(base_a[,2:54],2,function(x){base_a=x/as.numeric(dias_uteis[,2]);return(base_a)})
...
base=cbind(base[1],apply(base[,2:54],2,function(x) x*(tail(deflator,1)/deflator)))
...
names(contribuicao)=c("Data", "Concessões - Pessoas jurídicas - Desconto de duplicatas e recebíveis - Recursos Livres -  20366",
...
write.csv2(contribuicao,"Contribuicoes concessoes para o total menos crédito a vista.csv", row.names = F)
